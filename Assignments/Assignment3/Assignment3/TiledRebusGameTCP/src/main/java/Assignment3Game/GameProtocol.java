        ...
        try {
            ...
            if (intCurrentState == WAITING) { // Send first question to client  
                ...
                if (msg.getState() == ANS_SENT) { //If client has sent answer
                        ...
                        if(msg.getText().equalsIgnoreCase(puzzleAnswers[0])){ // Check if client has entered rebus puzzle answer           
                            strMsg = "You Win!"; 
                            intQuestionFlag = 0; 
                            intCurrentState = WIN;
                            return (prepareMsg(msg, strMsg, intQuestionFlag, intCurrentState)); //break out if puzzle answer is correct.
                        } else if (boolAnswerCorrect && intImageIndex < (intNumTiles*intNumTiles)-1) { // Check if answer to qn is correct and if all images have been sent.
